...
/*}}}*/
struct String *Value_toString(struct Value *this, struct String *s, char pad, int headingsign, size_t width, int commas, int precision, int exponent, int trailingsign) /*{{{*/
{
  ...
  switch (this->type)
  {
    ...
    case V_INTEGER:
    {
      ...
      if (totalwidth>buf.length) memset(s->character+oldlength,pad,totalwidth-buf.length);
      memcpy(s->character+oldlength+(totalwidth>buf.length?(totalwidth-buf.length):0),buf.character,buf.length);
      String_destroy(&buf);
      break;
    }
    case V_STRING:
...
struct Value *Value_toStringUsing(struct Value *this, struct String *s, struct String *using, size_t *usingpos) /*{{{*/
{
  ...
  work:
  Value_toString(this,s,pad,headingsign,width,commas,precision,exponent,trailingsign);
