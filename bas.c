...
static struct Value *lvalue(struct Value *value) /*{{{*/
{
  ...
  if ((pc.token+1)->type==T_OP)
  {
    ...
    while (1)
    {
      ...
      {
        idx=(int*)realloc(idx,sizeof(unsigned int)*(capacity?(capacity*=2):(capacity=3)));
      }
      /*}}}*/
      idxpc=pc;
...
static void allowBreak(int on) /*{{{*/
{
  ...
  breakact.sa_handler=on ? dobreak : SIG_IGN;
...
static void pushLabel(enum LabelType type, struct Pc *patch) /*{{{*/
{
  ...
  {
    labelStack=realloc(labelStack,sizeof(struct LabelStack)*(labelStackCapacity?(labelStackCapacity*=2):(32)));
  }
...
static struct Value *assign(struct Value *value) /*{{{*/
{
  ...
  if (pass!=DECLARE && VALUE_RETYPE(value,l->type)->type==V_ERROR)
  {
    pc=expr;
    return value;
  }
...
void bas_init(int backslash_colon, int uppercase, int lpfd) /*{{{*/
{
#ifdef HAVE_GETTEXT
...
{
  FS_putChars(0,"bas " VERSION "\n");
  FS_putChars(0,"Copyright 1999-2010 Michael Haardt.\n");
  FS_putChars(0,_("This is free software with ABSOLUTELY NO WARRANTY.\n"));
  new();
  while (1)
  {
    struct Token *line;
    ...
    FS_flush(STDCHANNEL);
    String_new(&s);
    if (FS_appendToString(STDCHANNEL,&s,'\n',1)==-1)
    {
      if (stop)
      {
