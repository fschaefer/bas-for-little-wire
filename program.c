...
/*}}}*/
int Program_delete(struct Program *this, long int line) /*{{{*/
{
  int i;
  ...
  for (i=0; i<this->size; ++i)
  {
    ...
    if (line==this->code[i]->u.integer)
    {
      Token_destroy(this->code[i]);
      if (i+1!=this->size) memmove(&this->code[i],&this->code[i+1],(this->size-i-1)*sizeof(struct Token*));
      --this->size;
      return 0;
    }
  }
...
struct Value *Program_merge(struct Program *this, int dev, struct Value *value) /*{{{*/
{
  ...
  l=0;
  while (String_new(&s),(err=FS_appendToString(dev,&s,'\n',1))!=-1 && s.length)
  {
    struct Token *line;
...
/*}}}*/
struct Value *Program_list(struct Program *this, int dev, struct Pc *from, struct Pc *to, struct Value *value) /*{{{*/
{
  int i,w;
  int indent=0;
  struct String s;
...
{
  char buf[80];
  register const struct Pc *pc=(const struct Pc*)k;
