...
/*}}}*/
#ifdef HAVE_TGETENT
static char *term,entrybuf[2048],capbuf[2048],*cap;
static char *cl,*cm,*ce,*md,*me,*AF,*AB;
static int Co,NC;
...
{
  if (initTerminal(chn)==-1) return -1;
  termchannel=chn;
  if (cl==(char*)0)
  {
...
int FS_opendev(int chn, int infd, int outfd) /*{{{*/
{
  ...
  file[chn]->dev=1;
  if ((file[chn]->tty=(infd==0 ? isatty(infd) : 0)))
  {
...
/*}}}*/
int FS_appendToString(int chn, struct String *s, char stop, int onl) /*{{{*/
{
  size_t new;
  char *n;
  struct FileStream *f=file[chn];
