...
struct Value *stmt_CALL(struct Value *value) /*{{{*/
{
  ...
  here=pc;
  name=pc.token->u.identifier->name;
  if (pass==DECLARE)
  {
        ...
        do
        {
          if (pc.token->type==T_IS) /*{{{*/
          {
            ...
            switch (pc.token->type)
            {
              ...
              case T_GE:
              case T_GT:
              case T_NE: relop=pc.token->type; break;
...
struct Value *stmt_DELETE(struct Value *value) /*{{{*/
{
  ...
  if (pc.token->type==T_INTEGER)
  {
    if (pass==INTERPRET)
    {
      if (!program.numbered) return Value_new_ERROR(value,UNNUMBERED);
      else if (pc.token->u.integer<=0) return Value_new_ERROR(value,OUTOFRANGE,_("line number"));
      else if (Program_delete(&program,pc.token->u.integer)==-1) return Value_new_ERROR(value,NOSUCHLINE);
    }
    ++pc.token;
  }
  return (struct Value*)0;
  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      if (pass==INTERPRET)
      {
        ...
        {
          geometry=realloc(geometry,sizeof(unsigned int)*(capacity?(capacity*=2):(capacity=3)));
        }
        /*}}}*/
...
struct Value *stmt_EDIT(struct Value *value) /*{{{*/
{
  ...
  if (pass==INTERPRET)
  {
    ...
    FS_width(chn,0);
    if (Program_list(&program,chn,(struct Pc*)0,(struct Pc*)0,value))
    {
      pc=statementpc;
      return value;
    }
...
{
  if (pass==INTERPRET) pc=pc.token->u.endpc;
  if (pass==DECLARE || pass==COMPILE)
  {
    Program_end(&program,&pc.token->u.endpc);
    ++pc.token;
  }
  return (struct Value*)0;
}
...
struct Value *stmt_LINEINPUT(struct Value *value) /*{{{*/
{
  ...
  if (pass==INTERPRET)
  {
    ...
    if (FS_appendToString(channel,&l->u.string,'\n',1)==-1) return Value_new_ERROR(value,IOERROR,FS_errmsg);
...
struct Value *stmt_LIST_LLIST(struct Value *value) /*{{{*/
{
  ...
  if (pass==INTERPRET)
  {
    ...
    if (Program_list(&program,channel,f?&from:(struct Pc*)0,t?&to:(struct Pc*)0,value)) return value;
  }
  return (struct Value*)0;
}
/*}}}*/
...
{
  struct Pc here=pc;
  const char *name=pc.token->u.identifier->name,*mid="mid$";

  while (*name && tolower(*name)==*mid) { ++name; ++mid; };
  if (tolower(*name)==*mid) /* mid$(a$,n,m)=b$ */ /*{{{*/
  {
    struct Value *l;
    long int n,m;
  ...
  else /* a=b or procedure call */ /*{{{*/
  {
    ...
    else
    {
      ...
      if (pc.token->u.identifier->sym->type==USERFUNCTION || pc.token->u.identifier->sym->type==BUILTINFUNCTION)
      {
        func(value);
        if (Value_retype(value,V_VOID)->type==V_ERROR) return value;
        Value_destroy(value);
...
struct Value *stmt_INPUT(struct Value *value) /*{{{*/
{
  ...
  if (pass==INTERPRET) /* read input line and tokenise it */ /*{{{*/
  {
    ...
    String_new(&s);
    if (FS_appendToString(channel,&s,'\n',nl)==-1) return Value_new_ERROR(value,IOERROR,FS_errmsg);
    ...
    if (pass==INTERPRET)
    {
      ...
      else if (convert(value,l,t))
      {
        ...
        if (channel==STDCHANNEL)
        {
          ...
          String_new(&s);
          Value_toString(value,&s,' ',-1,0,0,-1,0,0);
...
struct Value *stmt_MAT(struct Value *value) /*{{{*/
{
  ...
  {
    if (var1->type==T_STRING) return Value_new_ERROR(value,TYPEMISMATCH5);
    ++pc.token;
...
struct Value *stmt_NEXT(struct Value *value) /*{{{*/
{
  ...
  else /*{{{*/
  {
    ...
    while (1)
    {
      ...
      if (level) *next=realloc(*next,sizeof(struct Next)*(level+1));
      ...
      if (pc.token->type==T_IDENTIFIER)
      {
        if (strcmp(pc.token->u.identifier->name,(*next)[level].var.token->u.identifier->name))
        {
...
struct Value *stmt_OPEN(struct Value *value) /*{{{*/
{
  ...
  int mode=FS_ACCESS_NONE,newMode,lock=FS_LOCK_NONE;
  long int channel;
...
struct Value *stmt_OUT_POKE(struct Value *value) /*{{{*/
{
  ...
  if (pass==INTERPRET)
  {
    if ((out ? FS_portOutput : FS_memOutput)(address,value->u.integer)==-1)
    {
      pc=lpc;
      return Value_new_ERROR(value,IOERROR,FS_errmsg);
    }
...
struct Value *stmt_SAVE(struct Value *value) /*{{{*/
{
  ...
  ++pc.token;
  loadpc=pc;
  if (pc.token->type==T_EOL && program.name.length) name=program.name.character;
  else
  {
    name=(const char*)0;
    if (eval(value,_("file name"))->type==V_ERROR || Value_retype(value,V_STRING)->type==V_ERROR)
  ...
  if (pass==INTERPRET)
  {
    ...
    if ((chn=FS_openout(name ? name : value->u.string.character))==-1)
    {
      pc=loadpc;
      if (name==(const char*)0) Value_destroy(value);
      return Value_new_ERROR(value,IOERROR,FS_errmsg);
    }
    FS_width(chn,0);
    if (name==(const char*)0) Value_destroy(value);
  ...
  else if (name==(const char*)0) Value_destroy(value);
  return (struct Value*)0;
}
/*}}}*/
...
struct Value *stmt_SHELL(struct Value *value) /*{{{*/
{
  ...
  if (eval(value,(const char*)0))
  {
    ...
    if (pass==INTERPRET) switch (pid=fork())
    {
      case -1: return Value_new_ERROR(value,FORKFAILED,strerror(errno));
      case 0:
      {
